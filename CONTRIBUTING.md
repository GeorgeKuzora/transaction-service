...
При этом `alembic` создаст новый файл миграции в директории `alembic/versions/` где будут отражены изменения схеме базы данных если они были внесены через `sqlalchemy.orm`.
...
- `app/` — директория с кодом сервиса.
...
```shell
docker run --name transaction-service -p 127.0.0.1:8082:8000 transaction-service
```
